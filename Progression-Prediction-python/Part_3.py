...
# Dictionary for print labels in text file
outcome_display = {
    "Progress": "Progress",
...
# Function to generate a unique file name for saving data
def create_file_name():
    """
    Creates a unique file name for saving data.

    Returns:
    - str: The unique file name.
    """
    prefix = "progression_data_"
    suffix = 1
    while True:
        file_name = f"{prefix}{suffix:03d}.txt"
        if not os.path.exists(file_name):
            return file_name
        suffix += 1

# Function to save student data to a file
def save_to_file(student_data, file_name):
    """
    Saves student data to a text file.

    Args:
    - student_data (list): A list of tuples containing student data.
    - file_name (str): The name of the file to save data.
    """
    with open(file_name, 'w') as file:
        if not student_data:
            file.write("There are no outcomes\n")
        else:
            for index, (outcome, pass_credits, defer_credits, fail_credits) in enumerate(student_data, start=1):
                file.write(f"{index:02d}. {outcome:<36} - {pass_credits:>5} {defer_credits:>5} {fail_credits:>5}\n")
    ...
    while True:
        ...
        while True:
            ...
            if continue_input == 'q':
                ...
                save_to_file(student_data, file_name)
                print("Part 3\n")
                read_file(file_name)
                print("\n============================================================================================")
                return
